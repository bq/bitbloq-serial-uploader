...
var state="Loading"; // state of the serial connection
var trimmed_commands=transform_data(); // trimmed_commands store the hex commands that will be passed to the board. 

var connectionId = -1; // ID of the serial connection (used by all the chrome serial API functions)
...
var address_l= new Array; 
var address_r= new Array; 
...
function buildPortPicker(ports) {
  ...
  portPicker.onchange = function() {
    ...
    console.log(devices); 
    openSelectedPort(devices);
  	});
  };
}

function openSelectedPort(devices) {
  //select port
  var portPicker = document.getElementById('port-picker');
  ...
  for (var i=0; i<devices.length; i++){
  		if(devices[i].path==selectedPort){
	  		if(devices[i].displayName=="Arduino_Uno"){
				bitrate=115200;
				delays=[300,300,300,30,70,5,30,70];
				max_size=32256;
				console.log("SELECTED BOARD Arduino UNO on port ", devices[i].path, "with bitrate=", bitrate);
			}
		 else if(devices[i].displayName=="FT232R_USB_UART"){
				bitrate=19200; 
				max_size=28672;
				delays=[300,300,300,50,90,20,100,70];
				console.log("SELECTED BOARD ZUM BT on port ", devices[i].path, "with bitrate=", bitrate);
			}
		}
	}
 	
  	  //connect to port
  console.log("Openning port with bitrate=", bitrate); 
  chrome.serial.connect(selectedPort,{bitrate:bitrate}, onConnect);
}
...
// Slice the used information from the input hex file
	...
	var prog=new Array(); 
	for (var i=0; i<prog_init.length; i++){
...
function transform_data(){
	...
	var i=0;
	var trimmed_commands=new Array; 
		...
		function(resolve, reject){
			...
			setTimeout(function(){			
				...
				console.log("DTR-RTS ON"); 
						...
						setTimeout(function(){resolve();}, delays[0]); 
					},delays[1]);	
				}, delays[2]); 		
	})
	p.then(
...
function enter_progmode(){
	var p=new Promise(
		function(resolve, reject){
			...
			setTimeout(function(){resolve();}, delays[3]); 
		})
	...
	var p=new Promise(
		function(resolve, reject){
			...
			chrome.serial.send(connectionId, load_address.buffer, function (){});
  			...
  			setTimeout(function(){ resolve();}, delays[4]); 
  			}); 
  	p.then(function(){
...
function program_page(it){
	var p=new Promise(
		function(resolve, reject){
			...
			setTimeout(function(){resolve();}, delays[5]); 
...
function serialSendPage(buffer, it){
	var p= new Promise(
		function(resolve, reject){
			...
			chrome.serial.send(connectionId, buffer.buffer, function (){});
			setTimeout(function(){resolve();},delays[6]); 
		});
	p.then(
		function(){
...
function leave_progmode(){	
	var p= new Promise(
		function(resolve,reject){
			...
			setTimeout(function(){resolve();}, delays[7]); 
		})
...
document.querySelector('button').addEventListener('click', function() {
	...
	console.log("Program size: ",sizeof(trimmed_commands), ". Max size available in the board: ", max_size);
	if(sizeof(trimmed_commands)<max_size){
		if(status == "Connected"){
		  	//Start the whole programming flow 
			changeSignals(0);
		}else{
		...
		console.log("The size of the program is larger than the memory available in the Flash memory"); 
	}
});
